...
*/
if($tdb->is_logged_in()) {
	$wRaw = $_COOKIE['user_env'].str_repeat(' ', 20 - strlen($_COOKIE['user_env'])).
	$_COOKIE['id_env'].str_repeat(' ', 16 - strlen($_COOKIE['id_env'])).
	$_COOKIE['power_env'];
	$wSearch =  $_COOKIE['id_env'].str_repeat(' ', 16 - strlen($_COOKIE['id_env']));
} else {
    $wRaw = 'guest'.str_repeat(' ', 15).
	getenv("REMOTE_ADDR").str_repeat(' ', 16 - strlen(getenv("REMOTE_ADDR"))).
	'0';
...
function whos_online($whos_online_log, $_STATUS) {
	...
	for($i=0;$i<$whos_online_count;$i++) {
		...
		if($wUser['user_power'] == '0') {
			...
			$return['who'][] = '<a href="profile.php?action=get&id='.$wUser['user_id'].'"><font color='.$color.'>'.$wUser['user_name'].'</font></a>';
		}
	}
	if(!empty($return['who'])) {
