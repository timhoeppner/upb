...
$pRec = $posts_tdb->get("posts", $_POST["p_id"]);
...
if(!(isset($_POST["id"]) && isset($_POST["t_id"]) && isset($_POST["p_id"]))) exitPage("Not enough information to perform this function.");
...
$msg = format_text(filterLanguage(UPBcoding(utf8_decode($_POST["message"])), $_CONFIG["censor"]));
$dbmsg = htmlentities(stripslashes(utf8_decode($_POST["message"])));

$posts_tdb->edit("posts", $_POST["p_id"], array("message" => $dbmsg, "edited_by_id" => $_COOKIE["id_env"], "edited_by" => $_COOKIE["user_env"], "edited_date" => mkdate()));
//clearstatcache();
$posts_tdb->cleanup();
$posts_tdb->setFp("posts", $_POST["id"]);
$pRec2 = $posts_tdb->get("posts", $_POST["p_id"]);
