...
if (!isset($_GET['t_id'])) {
    ...
    foreach ($reverse as $key => $pRec) {
      ...
      $xml .= "
      <item>
        <title>Reply on ".xml_clean(gmdate("M d, Y @ g:i:s a", user_date($pRec["date"])))."</title>
    		<author>".xml_clean($pRec['user_name'])."</author>
    		<pubDate>".xml_clean(date('D, d M Y H:i:s e', ($pRec['date']-(6*60*60))))."</pubDate>
        <link>".xml_clean($newurl)."</link>
