...
if($tdb->is_logged_in() === false) exitPage("You are not logged in, therefore unable to perform this action.");
if(!isset($_GET["id"]) || !isset($_GET["t_id"]) || ($_GET["t"] == 0 && !isset($_GET["p_id"]))) exitPage("Not enough information to perform this function.");
if($_COOKIE["power_env"] < 2 && $_GET['t'] != 0) exitPage("You do not have enough power to delete this topic");

if($_GET['action'] != "delete") exitPage("Unknown Action.  Seek Administrative Help.");
...
if($_GET["t"] == 1) {
	...
	if($_POST["verify"] == "Ok") {
		...
		if(($key = array_search($_GET["p_id"], $p_ids)) !== FALSE) {
			$update_topic = array("replies" => ((int)$tRec[0]["replies"] - 1), "p_ids" => $tRec[0]["p_ids"]);
			if($key == (count($p_ids) - 1)) { //last post, update last_post of topic
		...
		echo "<table width='".$_CONFIG["table_width_main"]."' cellspacing='1' cellpadding='3' bgcolor='$border' align='center'>
                    <tr><td bgcolor='$header' align=left valign=center><font size='$font_m' face='$font_face' color='$font_color_header'>Author:</font></td>
                    <td bgcolor='$header' align=left valign=center><font size='$font_m' face='$font_face' color='$font_color_header'> </font></td></tr>";
		$table_color = $table1;
		$table_font = $font1;
		$user = $tdb->get("users", $pRec[0]["user_id"]);
		if($user[0]["sig"] != "") $user[0]["sig"] = "<br><table width='70%' class='signature' cellspacing='0' cellpadding='4'>
                    <tr><td>".UPBcoding(filterLanguage($user[0]["sig"], $_CONFIG["censor"]))."</td></tr></table>";
		...
		echo "<tr><td bgcolor='$table_color' valign=top width=15%><font size='$font_m' face='$font_face' color='$table_font'>
                    <font color=$statuscolor><b>&nbsp;<a href='profile.php?action=get&id=".$pRec[0]["user_id"]."'>".$pRec[0]["user_name"]."</a></b></font><br>&nbsp;$status";
		...
		echo "<br><font size='$font_s'>&nbsp;Posts: ".$user[0]["posts"]."
		...
		echo"
                    ...
                    <td height='1%'><br>".$user[0]["sig"]."</td></tr></table></TD></tr>";
