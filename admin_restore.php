...
if(!($_GET['action'] == 'download' && isset($_GET['file'])) && $_POST['verify'] != 'Cancel') {
    ...
    echo "<tr><td background=".$_CONFIG["skin_dir"]."/images/title_bg.gif bgcolor='$header'><B><font size='$font_l' face='$font_face' color='$font_color_header'>Backup and Restore Data</font></b></td></tr>";
    echo "<tr><td bgcolor='$table1' width=50%><font size='$font_m' face='$font_face' color='$font_color_main'>";

    echo "<a href='admin_restore.php?action=backup'>Backup the current database</a><br>
    ...
    <a href='admin_restore.php?action=delete'>Delete all backups</a>
    </td></tr>";
    
    if(isset($_GET['action'])) echo '<tr><td bgcolor="'.$table1.'">';
}
if($_GET['action'] == 'backup') {
    ...
    echo 'Successfully backed up the current database to <b>'.DB_DIR.'/backup/'.$filename.'</b>.  Would you like to <a href="admin_restore.php?action=download&file='.DB_DIR.'/backup/'.$filename.'">download it?</a>';
    ...
    if(!isset($_GET['file'])) {
        ...
        if(!isset($_POST['verify'])) ok_cancel('admin_restore.php?action=import&file='.$_GET['file'], 'Are you sure you wish to import <b>'.$_GET['file'].'</b>?');
        elseif($_POST['verify'] == 'Cancel') redirect('admin_restore.php', 0);
        elseif($_POST['verify'] == 'Ok') {
            if(rename('./backup/'.$_GET['file'], DB_DIR.'/backup/'.$_GET['file'])) echo 'Successfully imported <b>'.$_GET['file'].'</b>.  Note this backedup database was not "restored" or loaded.';
...
echo "</td></tr></table>".$skin_tablefooter;
